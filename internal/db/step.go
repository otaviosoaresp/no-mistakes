...
// readableStepResultColumns selects every column added after the original
// schema only when it is actually present. Read-only preflight can inspect a
// database before migrations run, and naming a missing column there fails the
// query outright, which would block every pipeline-control command.
//
// The ci_fix_attempts column is no longer read: the CI step's fix rounds are
// counted by the executor from the round history, exactly like every other
// step's, and the column only remains because migrations are append-only.
func (d *DB) readableStepResultColumns() string {
	columns := stepResultColumns
	if d.hasColumn("step_results", "max_rounds") {
		columns += ", max_rounds"
	} else {
		columns += ", NULL AS max_rounds"
	}
...
func (d *DB) GetStepResult(id string) (*StepResult, error) {
	...
	).Scan(&s.ID, &s.RunID, &s.StepName, &s.StepOrder, &s.Status, &s.ExitCode, &s.DurationMS, &s.LogPath, &s.FindingsJSON, &s.Error, &s.StartedAt, &s.CompletedAt, &s.LastActivityAt, &s.LastActivity, &s.AgentPID, &s.AutoFixLimit, &s.MaxRounds, &s.RoundStartedAt, &s.OverrideReason, &s.SkipReason)
...
func (d *DB) GetStepsByRun(runID string) ([]*StepResult, error) {
	...
	for rows.Next() {
		s := &StepResult{}
		if err := rows.Scan(&s.ID, &s.RunID, &s.StepName, &s.StepOrder, &s.Status, &s.ExitCode, &s.DurationMS, &s.LogPath, &s.FindingsJSON, &s.Error, &s.StartedAt, &s.CompletedAt, &s.LastActivityAt, &s.LastActivity, &s.AgentPID, &s.AutoFixLimit, &s.MaxRounds, &s.RoundStartedAt, &s.OverrideReason, &s.SkipReason); err != nil {
...
func (d *DB) ResetStepsFrom(runID string, stepOrder int) error {
	_, err := d.sql.Exec(`
		...
		SET status = ?, exit_code = NULL, duration_ms = NULL, log_path = NULL,
			...
			agent_pid = NULL, auto_fix_limit = NULL, max_rounds = NULL
...
func (d *DB) StartStep(id string) error {
	return d.StartStepWithLimits(id, 0, 0)
}

// StartStepWithLimits marks a step as running and records the effective
// auto-fix limit and total round budget that status surfaces use while the
// step is active. Either limit is stored as NULL when it is unlimited.
func (d *DB) StartStepWithLimits(id string, autoFixLimit, maxRounds int) error {
	ts := now()
	_, err := d.sql.Exec(`UPDATE step_results SET status = ?, started_at = ?, round_started_at = ?, last_activity_at = ?, last_activity = ?, agent_pid = NULL, auto_fix_limit = ?, max_rounds = ? WHERE id = ?`, types.StepStatusRunning, ts, ts, ts, "step started", stepLimitDBValue(autoFixLimit), stepLimitDBValue(maxRounds), id)
	if err != nil {
...
func (d *DB) StartStepFixRound(id string, autoFixLimit int) error {
	ts := now()
	_, err := d.sql.Exec(`UPDATE step_results SET status = ?, round_started_at = ?, last_activity_at = ?, last_activity = ?, auto_fix_limit = ? WHERE id = ?`, types.StepStatusFixing, ts, ts, fmt.Sprintf("status: %s", types.StepStatusFixing), stepLimitDBValue(autoFixLimit), id)
	if err != nil {
...
func (d *DB) SetStepAutoFixLimit(id string, autoFixLimit int) error {
	if _, err := d.sql.Exec(`UPDATE step_results SET auto_fix_limit = ? WHERE id = ?`, stepLimitDBValue(autoFixLimit), id); err != nil {
...
// stepLimitDBValue stores a non-positive limit as NULL, so "unlimited" reads
// back the same way for rows written before the column existed.
func stepLimitDBValue(limit int) any {
	if limit <= 0 {
		return nil
	}
	return limit
