...
When `CLAUDE_CONFIG_DIR` is set, the Claude Code copy follows it to `$CLAUDE_CONFIG_DIR/skills/no-mistakes/SKILL.md` instead of `~/.claude/skills`, so a profile setup that points Claude Code at a non-default config directory gets the skill in the profile it actually reads. A relative value is resolved against the working directory. The vendor-neutral `~/.agents/skills` copy is unaffected.
